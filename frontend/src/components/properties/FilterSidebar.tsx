...
import { Apartment, House, Villa, Building, MapPin, Check } from 'lucide-react';
...
<div className="relative mb-3">
    <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-primary text-lg" />
    <input
      type="text"
...
<button
  ...
  className={`flex-1 h-11 rounded-xl border font-manrope font-bold text-sm transition-all ${
    selectedAvailability === avail.id
      ? 'bg-[#D4755B] border-[#D4755B] text-[#F9FAFB] shadow-md'
      : 'bg-white border-[#E6E0DA] text-[#374151] hover:border-[#D4755B] hover:text-[#D4755B]'
  }`}
>
  {avail.label}
</button>
...
>
  <Building className="text-2xl text-[#374151] hover:text-[#D4755B] hover:scale-110 transition-all" />
...
{selectedAmenities.includes(amenity) && (
  <Check className="text-[#D4755B] text-xs font-bold" />
)}
                </div>
                <input
                  type="checkbox"
...
<span className={`font-manrope text-sm ${
  selectedAmenities.includes(amenity) ? 'text-[#D4755B] font-semibold' : 'text-[#374151]'
}`}>
  {amenity}
</span>
              </label>
        ...
        <button
          onClick={handleReset}
          className="w-full bg-white border border-[#D4755B] text-[#D4755B] font-manrope font-bold text-base py-4 rounded-xl transition-all hover:bg-[#D4755B] hover:text-[#F9FAFB] sticky bottom-0"
        >
          Reset Filters
        </button>
      </div>
