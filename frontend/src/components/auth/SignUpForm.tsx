...
const SignUpForm: React.FC<SignUpFormProps> = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 text-[#94A3B8] hover:text-[#D4755B] transition-colors"
          >
            {showConfirmPassword ? <EyeOff className="w-5 h-5 text-primary" /> : <Eye className="w-5 h-5 text-primary" />}
          </button>
        </div>
      </div>
      ...
      <button
        type="submit"
        className="w-full bg-[#D4755B] hover:bg-[#C05621] text-[#F9FAFB] font-manrope font-bold text-base py-3.5 rounded-xl transition-all shadow-lg hover:shadow-xl"
      >
        Create Account
      </button>
    </form>
